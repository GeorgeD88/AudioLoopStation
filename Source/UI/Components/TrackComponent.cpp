...
// Monolithic individual track transport that includes buttons sliders and toggles for refactored LoopTrack
//
...
TrackComponent::TrackComponent(AudioLoopStationAudioProcessor& p, LoopTrack& trackToControl, int trackIndex)
        : processor(p), track(trackToControl), trackID(trackIndex)
{
    ...
    setupButton(recPlayButton);
    setupButton(stopButton);
    setupButton(undoButton);
    setupButton(afterLoopButton);
    setupButton(clearButton);
    setupButton(fxReplaceButton);
